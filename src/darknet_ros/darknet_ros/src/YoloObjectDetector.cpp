...
namespace darknet_ros {
    ...
    char *data;
    char **detectionNames;
//cv::Mat camera_temp_ = cv::Mat::zeros(1280, 960, CV_8UC3);
//cv::Mat temp = cv::Mat::zeros(1280,1024,CV_8UC3);
    cv::Rect roi_rect_(0, 32, 1280, 960);
        ...
        if (cam_image) {
            {
                ...
                //temp = cam_image->image.clone();
                camImageCopy_ = cam_image->image.clone();
                //camera_temp_ = temp(roi_rect_);
                //cv::resize(camera_temp_, camImageCopy_, cv::Size(), 0.5, 0.5);
            }
            {
            ...
            frameWidth_ = camImageCopy_.size().width;
            frameHeight_ = camImageCopy_.size().height;
            //std::cout << frameWidth_ << "  " << frameWidth_ << std::endl;
        }
        return;
    }
