...
//   if(img_queue.size()>QUEUE_MAX)
//        img_queue.pop();
//    img_queue.push(cv_bridge::toCvShare(img_msg, "bgr8")->image.clone());
            capture_begin_ = std::chrono::high_resolution_clock::now();
        ...
        int NextImage(cv::Mat &src_img) {
            ...
            if (buffer_state_[latest_index_] == BufferState::WRITE) {
                ...
                /*else if(!img_queue.empty())
                {
                    src_img=img_queue.front();
                    img_queue.pop();
                    lock_.unlock();
                    return 4;
                }*/
            else {
                ROS_INFO_THROTTLE(1, "No image is available");
                lock_.unlock();
                return temp_index;
            }
        ...
        cv::Mat DistillationColor(const cv::Mat &src_img, unsigned int color, bool using_hsv) {
            if (using_hsv) {
                ...
                if (color == 1) {
                    cv::Mat result_img;
                    // cv::Mat average = (bgr[1] + bgr[0]) / 2.0;
                    cv::subtract(bgr[2], bgr[1], result_img);
                    return result_img;
                } else if (color == 0) {
                    cv::Mat result_img;
                    // cv::Mat average = (bgr[1] + bgr[2]) / 2.0;
        ...
        void DrawRotatedRect(const cv::Mat &img, const cv::RotatedRect &rect, const cv::Scalar &color, int thickness) {
            ...
            // cv::putText(img, text, center, font_face, font_scale, cv::Scalar(0, 0, 255), thickness, 8, 0);
        ...
        void DrawRotatedRect(const cv::Mat &img,
            ...
            cv::Point2f vertex[4];
            vertex[0] = rect.tl();
            vertex[1] = vertex[0] + cv::Point2f(rect.width, 0);
            vertex[2] = rect.br();
            vertex[3] = vertex[2] - cv::Point2f(rect.width, 0);
            for (int i = 0; i < 4; i++)
                cv::line(img, vertex[i], vertex[(i + 1) % 4], color, thickness);
        }

        // --------------------------
        // Draw 4 vertexes of armor
        // --------------------------
        void DrawRotatedRect(const cv::Mat &img,
            ...
            for (int i = 0; i < 4; i++)
                cv::line(img, vertexes[i], vertexes[(i + 1) % 4], color, thickness);
        }
        // ------------------------
