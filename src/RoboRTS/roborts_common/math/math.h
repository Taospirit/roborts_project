...
//use cross judge convex
    ...
    inline double PointToLineDistance(const Eigen::Ref<const Eigen::Vector2d> &point,
        ...
        double
                u =
                ((point.x() - line_start.x()) * diff.coeffRef(0) + (point.y() - line_start.y()) * diff.coeffRef(1)) /
                sq_norm;
    ...
    inline double DistancePointToPolygon2D(const Eigen::Vector2d &point, const std::vector<Eigen::Vector2d> &vertices) {
        ...
        for (int i = 0; i < (int) vertices.size() - 1; ++i) {
            ...
            double new_dist = PointToLineDistance(point.coeffRef(0),
                                                  point.coeffRef(1),
                                                  vertices.at(i).coeffRef(0),
                                                  vertices.at(i).coeffRef(1),
                                                  vertices.at(i + 1).coeffRef(0),
                                                  vertices.at(i + 1).coeffRef(1));
            if (new_dist < dist) {
                dist = new_dist;
            }
        ...
        if (vertices.size() > 2) {
            double new_dist = PointToLineDistance(point.coeffRef(0),
                                                  point.coeffRef(1),
                                                  vertices.back().coeffRef(0),
                                                  vertices.back().coeffRef(1),
                                                  vertices.front().coeffRef(0),
                                                  vertices.front().coeffRef(1));
            if (new_dist < dist) {
