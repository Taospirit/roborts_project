...
extern int MVCameraParaLoad(char save_path[50], Width_Height *g_W_H_INFO);

extern void save_asprototxt(char save_paht[50]);
...
void MainWindow::camera_statues() {
    m_camera_statuesFps->setText(
            QString("Capture fps: %1  Display fps :%2").arg(QString::number(g_read_fps, 'f', 2)).arg(
                    QString::number(g_disply_fps, 'f', 2)));
    g_read_fps = 0;
    g_disply_fps = 0;
}
...
int MainWindow::init_SDK() {
    ...
    g_pRgbBuffer = (unsigned char *) malloc(
            g_tCapability.sResolutionRange.iHeightMax * g_tCapability.sResolutionRange.iWidthMax * 3);
...
//设置最大最小值 +10
    connect(ui->horizontalSlider_isp_acutance, SIGNAL(valueChanged(int)), ui->spinBox_isp_acutance,
            SLOT(setValue(int)));
    connect(ui->spinBox_isp_acutance, SIGNAL(valueChanged(int)), ui->horizontalSlider_isp_acutance,
            SLOT(setValue(int)));
...
int MainWindow::GUI_init_speed(int hCamera, tSdkCameraCapbility *pCameraInfo) {
    ...
    for (i = 0; i < size; i++) {
        ...
        radioButton_speed[i] =
                new QRadioButton(QString("%1").arg(pCameraInfo->pFrameSpeedDesc[i].acDescription), ui->groupBox_speed);
...
void MainWindow::on_pushButton_snap_catch_released() {
    ...
    if (CameraSnapToBuffer(g_hCamera, &tFrameHead, &pbyBuffer, 1000) == CAMERA_STATUS_SUCCESS) {
        switch (g_SaveImage_type) {
            ...
            case 3:
                pbImgBuffer = (unsigned char *) malloc(
                        g_tCapability.sResolutionRange.iHeightMax * g_tCapability.sResolutionRange.iWidthMax * 3);
                /*
...
void MainWindow::on_SetRes_clicked() {
    int width = ui->Width->text().toInt();
    int height = ui->Height->text().toInt();
    int w_offset = ui->W_Offset->text().toInt();
    int h_offset = ui->H_Offset->text().toInt();

    width = width < g_tCapability.sResolutionRange.iWidthMax ? width : g_tCapability.sResolutionRange.iWidthMax;
    height = height < g_tCapability.sResolutionRange.iHeightMax ? height : g_tCapability.sResolutionRange.iHeightMax;
    width = width > g_tCapability.sResolutionRange.iWidthMin ? width : g_tCapability.sResolutionRange.iWidthMin;
    height = height > g_tCapability.sResolutionRange.iHeightMin ? height : g_tCapability.sResolutionRange.iHeightMin;

    ui->Width->setText(QString::number(width));
    ui->Height->setText(QString::number(height));
    if (w_offset > g_tCapability.sResolutionRange.iWidthMax - width)
        w_offset = g_tCapability.sResolutionRange.iWidthMax - width;
    if (h_offset > g_tCapability.sResolutionRange.iHeightMax - height)
        h_offset = g_tCapability.sResolutionRange.iHeightMax - height;
    ui->W_Offset->setText(QString::number(w_offset));
    ui->H_Offset->setText(QString::number(h_offset));
    ...
    CameraSetImageResolution(g_hCamera, p);

    g_W_H_INFO.sensor_width = width;
    g_W_H_INFO.sensor_height = height;
    g_W_H_INFO.buffer_size = g_W_H_INFO.sensor_width * g_W_H_INFO.sensor_height;
...
void MainWindow::on_SetRes_2_clicked() {
    QString path = ui->prototxt_save->text();
    save_asprototxt(path.toLatin1().data());

}

void MainWindow::on_LoadResButton_clicked() {
    QString path = ui->prototxt_save->text();
    MVCameraParaLoad(path.toLatin1().data(), &g_W_H_INFO);
