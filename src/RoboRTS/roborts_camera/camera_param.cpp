...
namespace roborts_camera {
    ...
    CameraParam::CameraParam(std::string file_suffix) {
        LoadCameraParam(file_suffix);
    }

    void CameraParam::LoadCameraParam(std::string file_suffix) {
        Cameras camera_info;
        std::string file_name = ros::package::getPath("roborts_camera") + "/config/camera_param_" + file_suffix + ".prototxt";
        ...
        for (unsigned int index = 0; index < camera_num; index++) {
            ...
            cameras_param_[index].camera_numb = camera_info.camera(index).camera_numb();
            ...
            //Resize_Ratio
            cameras_param_[index].resize_ratio = camera_info.camera(index).resize_ratio();
