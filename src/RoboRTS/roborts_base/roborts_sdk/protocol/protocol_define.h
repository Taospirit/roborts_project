...
#define CMD_REFEREE_SEND_DATA               (0X01u)
    ...
    typedef struct {
        float robot_pos_x;
        float robot_pos_y;
        uint16_t robot_remain_hp;
        uint8_t robot_pjt_info;
        uint8_t find_enemy1;
        float enemy1_pos_x;
        float enemy1_pos_y;
        uint8_t find_enemy2;
    ...
    typedef struct {
        ...
        uint16_t receiver_id;
        cmd_robot_data_t data;
        uint8_t reserve[122-sizeof(cmd_robot_data_t)];
    } cmd_rev_student_data;
...
#define CMD_PUSH_GIMBAL_INFO           (0X01u)
    typedef struct {
        uint8_t mode;
        uint8_t gimbal_mode;
        uint8_t pjt_key;  //projectile info
...
/*----------------------------STUDENT_CMD----0X43----------------------*/
#define REFEREE_STUDENT_RECEIVE (0X01u)
