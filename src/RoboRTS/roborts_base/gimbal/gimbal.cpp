...
extern roborts_msgs::Debug global_debug_msg;
...
namespace roborts_base {
    ...
    void Gimbal::ROS_Init() {
        ...
        ros_sub_projectile_src_ = ros_nh_.subscribe("projectile_info", 1, &Gimbal::GimbalProjectileCallback, this);
        ros_sub_set_fric_speed_ = ros_nh_.subscribe("set_fric_speed", 1, &Gimbal::FricSpeedCallback, this);
        ...
        //ros_message_init
        gimbal_tf_.header.frame_id = robot_base_frame_;
        gimbal_tf_.child_frame_id = gimbal_frame_;

    }

    void Gimbal::FricSpeedCallback(const roborts_msgs::FricSpeed::ConstPtr &msg) {
        roborts_sdk::cmd_fric_wheel_speed fric_speed;
        fric_speed.left = msg->fric_speed;
        fric_speed.right = msg->fric_speed;
        fric_wheel_pub_->Publish(fric_speed);
    }

    void Gimbal::GimbalProjectileCallback(const roborts_msgs::ProjectileInfo::ConstPtr &msg) {
        //recieve msg: 1:no projectile; 2:few projectile 3:enough projectile
        uint8_t pjt_info = msg->pjt_info;
        lock_.lock();
        pjt_info_ = pjt_info;
        lock_.unlock();
    ...
    void Gimbal::GimbalInfoCallback(const std::shared_ptr<roborts_sdk::cmd_gimbal_info> gimbal_info) {
        ...
        global_debug_msg.gimbal_mode = gimbal_info->gimbal_mode;
    }
...
//! 返回上一帧子弹检测状态
//! 1：没弹（未上膛且弹仓检测不到） 2：少弹（已上膛且弹仓检测不到）  3：多弹（弹仓检测到）  0：错误调用 4：未检测
    bool Gimbal::CtrlShootService(roborts_msgs::ShootCmd::Request &req,
        ...
        uint8_t projectilestatus;
        lock_.lock();
        projectilestatus = pjt_info_;
        pjt_info_ = 4;
        lock_.unlock();
        res.received = projectilestatus;
        return true;
    }
}
